...
def get_categorical_column_indexes(
    df, subset_cols=None, ignore_cols=None, threshold=10
):
    """
...
class Encoding(BaseEstimator):
    ...
    def apply_encoding(self, X_in, encoding_dict):
        ...
        for col in self.categorical_columns:
            if col in encoding_dict:
                ...
                X[col] = X[col].apply(
                    lambda x: freq_dict[x] if x in freq_dict else np.nan
                )
        return X
    ...
    def save_encoding(self, file_name):
        assert (
            self.encoding_dict and len(self.encoding_dict.keys()) > 0
        ), "Cannot save a model that is not fitted"
...
class DummyEncoding(Encoding):
    ...
    def __init__(
        self, categorical_columns=None, return_df=False, drop_original=True, threshold=0
    ):
    ...
    def create_encoding_dict(self, X, y):
        ...
        return {
            col: [
                k
                for k, v in list(
                    X[col].astype("object").value_counts().to_dict().items()
                )
                if v > self.threshold
            ]
            for col in self.categorical_columns
    ...
    def apply_encoding(self, X_in, encoding_dict):
        ...
        for col, values in list(encoding_dict.items()):
            for val in values:
                X["{}_{}".format(col, val)] = (
                    X[col].astype("object").apply(lambda x: 1 if x == val else 0)
                )
...
class ThresholdEncoding(Encoding):
    def __init__(
        self, categorical_columns=None, return_df=False, threshold=1, others="others"
    ):
        self.categorical_columns = categorical_columns
        self.return_df = return_df
        self.threshold = threshold
    ...
    def apply_encoding(self, X_in, encoding_dict):
        ...
        for col in self.categorical_columns:
            if col in encoding_dict:
                freq_dict = encoding_dict[col]
                X[col] = X[col].apply(
                    lambda x: freq_dict[x] if x in freq_dict else self.others
                )
        return X
    ...
    def create_encoding_dict(self, X, y=None):
        ...
        for col in self.categorical_columns:
            ...
            encoding_dict.update(
                {col: {k: k for k, v in list(vc.items()) if v > self.threshold}}
            )
        return encoding_dict
...
class FreqeuncyEncoding(Encoding):
    ...
    def __init__(
        self, normalize=1, categorical_columns=None, return_df=False, **kwargs
    ):
        self.categorical_columns = categorical_columns
        self.return_df = return_df
...
class BaseNEncoding(Encoding):
    ...
    def __init__(
        ...
        if base < 1 or base > 10:
            raise ValueError(
                "Either base is less than 1 or greater than 10 or n is less than 0"
            )
        self.base = base
        self.categorical_columns = categorical_columns
        self.return_df = return_df
    ...
    def create_encoding_dict(self, X, y):
        ...
        for col in self.categorical_columns:
            encoding_dict.update(
                {col: {x: i + 1 for i, x in enumerate(pd.unique(X[col]))}}
            )
        return encoding_dict
    ...
    def toStrOfBase(self, n, base):
        ...
        if n == 0:
            return "0"
        if base == 1:
            return "".join(["1" if i == 0 else "0" for i in range(n)])
        if n < base:
            return str(n)
        else:
    ...
    def apply_encoding(self, X_in, encoding_dict):
        ...
        for col in self.categorical_columns:
            ...
            X["tmp_" + str(col)] = X[col].apply(
                lambda x: self.toStrOfBase(freq_dict.get(x, 0), self.base).zfill(
                    _max_base_len
                )
            )
            for i in range(_max_base_len):
                X["col_%s_base%s_%d" % (str(col), str(self.base), i)] = X[
                    "tmp_" + str(col)
                ].str[_max_base_len - i - 1]
            del X["tmp_" + str(col)]
...
class TargetEncoding(Encoding):
    ...
    def __init__(
        ...
        min_samples_leaf=1,
        smoothing=1,
        noise_level=0.01,  # encoding tuning params
        **kwargs
    ):
        self.categorical_columns = categorical_columns
        self.return_df = return_df
    ...
    def create_encoding_dict(self, X, y):
        ...
        for col in self.categorical_columns:
            ...
            averages = (
                pd.DataFrame(list(zip(category_values, y)))
                .groupby(0)[1]
                .agg(["mean", "count"])
            ...
            for key, val in list(averages.items()):
                smoothing = 1.0 / (
                    1 + np.exp(-(val["count"] - self.min_samples_leaf) / self.smoothing)
                )
                enc_d.update(
                    {key: prior_mean * (1 - smoothing) + val["mean"] * smoothing}
                )
    ...
    def apply_encoding(self, X_in, encoding_dict):
        ...
        for col in self.categorical_columns:
            X[col] = X[col].apply(
                lambda x: encoding_dict[col].get(
                    x, encoding_dict["prior_mean"] * (1 - smoothing)
                )
            )
        return X
    ...
    def fit_transform(self, X, y):
        ...
        kf = CustomFolds(
            validation_scheme=self.fold_scheme,
        ...
        for i, (train_index, test_index) in enumerate(kf.split(X)):
            encoding_dict = self.create_encoding_dict(
                X.iloc[train_index], y.iloc[train_index]
            )
            copy_X.iloc[test_index] = self.apply_encoding(
                X.iloc[test_index], encoding_dict
            )
        if self.return_df:
            return copy_X
        else:
...
class WoeEncoding(Encoding):
    ...
    def __calculate_woe(self, y, val_indexes):
        ...
        """
        n_att_good, n_att_bad = np.count_nonzero(y[val_indexes] == 0), np.count_nonzero(
            y[val_indexes] == 1
        )
        n_tot_good, n_tot_bad = np.count_nonzero(y == 0), np.count_nonzero(y == 1)
        distr_good = (1.0 * n_att_good / n_tot_good) if n_tot_good > 0 else 0
        distr_bad = (1.0 * n_att_bad / n_tot_bad) if n_tot_bad > 0 else 0
        woe = (
            -20
            if distr_good == 0
            else 20
            if distr_bad == 0
            else max(min(np.log(distr_good / distr_bad), 20), -20)
        )
    ...
    def fit_transform(self, X, y):
        ...
        for i, (train_index, test_index) in enumerate(kf.split(copy_X)):
            encoding_dict, iv = self.create_encoding_dict(
    ...
    def apply_encoding(self, X_in, encoding_dict):
        ...
        for col in self.categorical_columns:
            ...
            for i, (a, b) in enumerate(zip(t[:-1], t[1:])):
                d = b - a
                X["col_bd_%s_%d" % (str(col), i)] = X["tmp_" + str(col)].apply(
                    lambda x: -1 * (d) if x <= d else d
                )
            del X["tmp_" + str(col)]
            del X[col]
        return X
...
class CatBoostEncoding(object):
    ...
    def catb_encoding(self, train, target, num_iter, cat_cols, prior):
        ...
        for col in cat_cols:
            ...
            for i in range(num_iter):
                ...
                li = []
                li = (
                    df.groupby(col)["target_val"].cumsum() - df["target_val"] + prior
                ) / (df.groupby(col).cumcount() + 1).values.tolist()
                cat_val_final.append(
                    pd.DataFrame({"cat_val": li, "indices": indices})
                    .sort_values("indices")
                    .cat_val.values.tolist()
                )

            cat_val_final = np.mean(cat_val_final, axis=0)
            map_dict = dict(
                pd.DataFrame({"col": df[col], "cat_val": cat_val_final})
                .groupby("col")["cat_val"]
                .mean()
            )
    ...
    def fit(self, train, target):
        self.encoding_dict = self.catb_encoding(
            train, target, self.num_iter, self.cat_cols, self.prior
        )
