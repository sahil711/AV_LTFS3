...
class CustomFolds(object):
    ...
    def split(
        self, X, y=None, group=None, **kwargs
    ):  # the group here will be passed on from the class where this is being called

        if (
            self.validation_scheme is None
            or isinstance(self.validation_scheme, KFold)
            or self.validation_scheme == FoldScheme.KFold.name
            or self.validation_scheme == FoldScheme.KFold
        ):
            folds = KFold(
            ...
            isinstance(self.validation_scheme, StratifiedKFold)
            or self.validation_scheme == FoldScheme.StratifiedKFold.name
            or self.validation_scheme == FoldScheme.StratifiedKFold
        ):
            ...
            isinstance(self.validation_scheme, GroupKFold)
            or self.validation_scheme == FoldScheme.GroupKFold.name
            or self.validation_scheme == FoldScheme.GroupKFold
        ):
            folds = GroupKFold(n_splits=self.num_folds)
            self.indices = [
                (train_index, test_index)
                for (train_index, test_index) in folds.split(X, y, groups=group)
            ]

        elif (
            isinstance(self.validation_scheme, TimeSeriesSplit)
            or self.validation_scheme == FoldScheme.TimeSeriesSplit.name
            or self.validation_scheme == FoldScheme.TimeSeriesSplit
        ):
            ...
            self.validation_scheme == FoldScheme.train_test_split.name
            or self.validation_scheme == FoldScheme.train_test_split
        ):
